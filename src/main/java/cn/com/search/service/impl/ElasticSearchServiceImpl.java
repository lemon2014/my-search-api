...
public class ElasticSearchServiceImpl implements ElasticSearchService {
	...
	public Boolean searchBulkIn(List<Map<String, Object>> datas, String index, String type) throws Exception {
		try {
			...
			for (Map<String, Object> data : datas) {
				...
				//  注意这里的_id, 默认是自动生成的字符串, 这里可以修改成数据对应的数据库id(分布式环境下也是唯一的id),可以快速的定位数据而且可以节省空间
	...
	//一般情况下es最多查询100页就不让查询了，因为是基于内存分页的，页就是会一次性把分页的数据全部加载到内存中，如果数据量比较大，就直接内存溢出了
	// 所以一定要控制页面查询的最大页数，就是百度的搜索引擎页也最多支持查询到几百页就不会显示更多的查询结果了
